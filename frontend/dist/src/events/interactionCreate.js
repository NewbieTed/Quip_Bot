    ...
    async execute(interaction) {
        ...
        if (interaction.customId.startsWith('create_thread_')) {
            ...
            try {
                ...
                const response = await (0, node_fetch_1.default)('http://localhost:8080/problem');
            ...
            const choiceRows = choices.map(choice => new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            ...
            collector.on('end', async (collected) => {
                ...
                if (collected.size === 0) {
                    try {
                        ...
                        await redis_1.redis.set(punishmentKey, 'true', 'EX', 3 * 60 * 60);
        ...
        if (interaction.customId.startsWith('answer_')) {
            ...
            try {
                ...
                problemDataRaw = await redis_1.redis.get(`problem:${problemId}`);
            }
            catch (err) {
